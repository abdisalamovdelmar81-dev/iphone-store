...
def product_text(product) -> str:
    color_line = f"Цвет: {product['color']}\n" if product["color"] else ""
    return (
        f"{product['name']}\n"
        f"{color_line}"
...
def setup_router(db: StoreDB, config: Config) -> Router:
    ...
    async def add_command(message: Message, command: CommandObject, state: FSMContext) -> None:
        ...
        if args:
            ...
            if quantity < 0 or price_value <= 0:
                await message.answer("Количество не меньше 0, цена больше 0.")
                return
            await state.update_data(name=name, quantity=quantity, price=price_value, color=color)
            await state.set_state(AddProductFlow.photo)
            await message.answer("Теперь отправь фото этого товара.")
            return

        await state.set_state(AddProductFlow.name)
        await message.answer("Напиши имя товара. Например: iPhone 16 Pro 256 ГБ")

    @router.message(F.text.func(lambda text: bool(text) and text.casefold() == "добавить"))
    ...
    @router.message(AddProductFlow.photo)
